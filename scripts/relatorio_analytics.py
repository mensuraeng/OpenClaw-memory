...
Gera relatório em HTML e envia via Telegram
"""
...
TELEGRAM_CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID", "-1003818163425")
...
def enviar_telegram_doc(html_path):
    if not TELEGRAM_TOKEN:
        print(f"HTML gerado: {html_path}")
        return
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendDocument"
    with open(html_path, "rb") as f:
        r = requests.post(url, data={"chat_id": TELEGRAM_CHAT_ID, "message_thread_id": TELEGRAM_THREAD_ID, "caption": "📊 Analytics Semanal — Mensura + MIA"}, files={"document": ("analytics_semanal.html", f, "text/html")})
    if not r.ok:
...
def main():
    ...
    if "--print" not in sys.argv:
        enviar_telegram_doc(OUTPUT_PATH)
