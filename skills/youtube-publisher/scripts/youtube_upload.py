...
def normalize_client_config(client_secret_file):
    """Aceita credenciais OAuth Desktop ou Web e converte para formato válido do InstalledAppFlow."""
    ...
    if "web" not in data:
        raise ValueError("Formato de client_secret.json inválido: esperado 'installed' ou 'web'.")
    ...
    normalized_path = os.path.join(SKILL_DIR, "client_secret.installed.json")
...
def get_authenticated_service():
    ...
    if not credentials or not credentials.valid:
        if credentials and credentials.expired and credentials.refresh_token:
            ...
            print("🔐 首次授权，将打开浏览器...")
            normalized_client_secret = normalize_client_config(CLIENT_SECRET_FILE)
            flow = InstalledAppFlow.from_client_secrets_file(normalized_client_secret, SCOPES)
