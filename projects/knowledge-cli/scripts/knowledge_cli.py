...
def status(args): emit({'mode':'automatic_v0.3','time':datetime.now().isoformat(timespec='seconds'),'scripts':{k:{'path':str(v),'exists':v.exists()} for k,v in SCRIPTS.items()},'guardrails':'inventory/read-only default; sync automatic for approved knowledge routine'},args)
def inventory(args):
...
def sync_authorized(args,script_key):
 ...
 emit({'mode':'automatic_execute_result','execute':True,'script':str(script),'ok':res['ok'],'code':res['code'],'stdout':res['stdout'][-8000:],'stderr':res['stderr'][-8000:]},args)
...
def guardrails(args): emit({'mode':'automatic_v0.3','rules':['Inventory/read-only por padrão.','Notion/NotebookLM/sync_knowledge podem rodar automaticamente com --execute para esta rotina aprovada.','Sem delete/upload/send.','Não versionar segredos.']},args)
def add_auth(x):
 x.add_argument('--execute',action='store_true'); x.add_argument('--timeout',type=int,default=600); x.add_argument('--json',action='store_true')
def parser():
