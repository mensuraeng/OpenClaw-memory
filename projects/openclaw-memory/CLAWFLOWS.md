# CLAWFLOWS.md — Operação de Workflows ClawFlows (referência sob demanda)

_Criado em 2026-04-17 a partir do bloco `<!-- clawflows:start -->...<!-- clawflows:end -->` que vivia dentro de `AGENTS.md` e estava sendo truncado pelo doctor (23%)._

> ⚠️ **Não rode `clawflows sync-agent` apontando para o `main` (Flávia).**
> Esse comando reinjeta o bloco genérico em `AGENTS.md` e quebra o boot. O conteúdo abaixo é a fonte de verdade da Flávia para ClawFlows. Ele é mantido manualmente. Se atualizar workflows com `clawflows update`, releia este arquivo — não o `AGENTS.md`.

---

<!-- BEGIN clawflows-content (extraído manualmente, não auto-gerar aqui) -->
