...
def memory_search_safe(query, base_path="/root/.openclaw/workspace/memory"):
    ...
    Nao depende de funcao externa memory_search().
    Faz busca direta por glob no filesystem de memoria.
    """
    results = []
    for pat in ["context/*.md", "*.md", "projects/*.md"]:
        for f in sorted(glob.glob(os.path.join(base_path, pat))):
            try:
                ...
                pass
    return results if results else [{'file': 'NONE', 'snippet': 'Sem resultados'}]
...
SPECIAL_QUERIES = {
    ...
    "quais decisoes continuam abertas": "open_decisions",
...
def query_open_decisions(limit: int) -> dict:
    ...
    pending_signals = [block for block in pending
                       if any(token in normalize(block) for token in ["aguard", "pend", "bloque", "precisa", "falta"])]
    return {"recent_decisions": decision_blocks, "pending_signals": pending_signals[:limit]}
...
def query_growing_risks(limit: int) -> dict:
    ...
    for path in [FILES["projects"], FILES["lessons"]]:
        for block in tail_blocks(path):
            hay = normalize(block)
            if any(token in hay for token in ["risco", "bloque", "falha", "pend", "atras",
                                               "sem resposta", "aguard", "critical_alert", "attention"]):
                risks.append(block)
    unique = []
    seen = set()
    for block in risks:
