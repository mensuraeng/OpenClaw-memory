...
def extract_daily_highlights(content):
    """Extrai destaques do arquivo diario.

    FIX: remove marcador de lista (- ou *) do inicio de cada linha
    antes de retornar, evitando duplo marcador quando build_estado_atual()
    adiciona '- ' na frente.
    """
    highlights = []
    for line in content.split('\n'):
        ...
        if re.search(r'entrega|alerta|concluido|urgente|\u2705|\u26a0', s, re.IGNORECASE):
            if s and len(s) > 10:
                # Remove marcador de lista do inicio (ex: "- texto" -> "texto")
                s_clean = re.sub(r'^[-*]\s+', '', s).strip()
                if s_clean and len(s_clean) > 10:
                    highlights.append(s_clean[:120])
    return highlights[:5]
