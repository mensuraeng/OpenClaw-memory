...
5. Verifica se o deploy público está respondendo (HEAD na URL).
   O deploy é gerenciado por Cloudflare Pages (domínio custom
   relatorios.<empresa>.com.br ou fallback *.pages.dev); o push
   no repo fonte aciona o build automático do CF Pages.
6. Notifica a Flávia via send_to_flavia com payload incluindo
   github_url (snapshot versionado) + relatorio_url (deploy público).
...
OBRAS = {
    "pg-louveira": {
        ...
        "relatorio_url": "https://relatorios.mensuraengenharia.com.br/",
        "relatorio_url_fallback": "https://p-g---louveira.pages.dev/",
...
def check_deploy(url: str, dry_run: bool) -> str:
    """HEAD no URL do deploy público. Retorna 'ativo' | 'inativo' | 'desconhecido'."""
    if dry_run:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            if 200 <= resp.status < 400:
                log(f"deploy ativo: {url} (status {resp.status})")
                return "ativo"
            log(f"deploy respondeu {resp.status} em {url}")
            return "inativo"
    except urllib.error.HTTPError as e:
        if e.code == 404:
            log(f"deploy 404 em {url} — provavelmente não está ativo")
            return "inativo"
        log(f"deploy HTTP {e.code} em {url}")
        return "inativo"
    except (urllib.error.URLError, OSError) as e:
        log(f"deploy inacessível em {url}: {e}")
        return "desconhecido"
...
def main() -> int:
    ...
    dominio = cfg["dominio"]
    relatorio_url = cfg["relatorio_url"]
    relatorio_url_fallback = cfg.get("relatorio_url_fallback")
    deploy_provider = cfg.get("deploy_provider", "unknown")
    ...
    deploy_status = check_deploy(relatorio_url, args.dry_run)
    ...
    payload = {
        ...
        "relatorio_url_fallback": relatorio_url_fallback,
        "deploy_provider": deploy_provider,
        "deploy_status": deploy_status,
        "scheduled_at": datetime.now().isoformat(),
        "body": (
            f"Relatório semana {semana} de {args.obra} ({dominio}) publicado.\n"
            f"Snapshot versionado: {github_context_url}\n"
            f"Deploy ({deploy_provider}, status={deploy_status}): {relatorio_url}"
        ),
    }
