...
def status(args): emit({'mode':'automatic_v0.3','time':datetime.now().isoformat(timespec='seconds'),'scripts':{k:{'path':str(v),'exists':v.exists()} for k,v in SCRIPTS.items()},'guardrails':'dry-run default; internal CCSP email automatic for approved MIA routine'},args)
def classify(args):
...
def controlled_script(args,kind):
 ...
 emit({'mode':'automatic_execute_result' if execute else 'dry_run_result','execute':execute,'kind':kind,'ok':res['ok'],'code':res['code'],'cmd':cmd,'stdout':res['stdout'][-6000:],'stderr':res['stderr'][-6000:]},args)
...
def weekly(args):
 ...
 emit({'mode':'automatic_execute_result' if execute else 'preview_only','execute':execute,'kind':'weekly','ok':res['ok'],'code':res['code'],'cmd':cmd,'stdout':res['stdout'][-6000:],'stderr':res['stderr'][-6000:]},args)
 if not res['ok']: raise SystemExit(res['code'])
def guardrails(args): emit({'mode':'automatic_v0.3','rules':['Dry-run é padrão.','Emails internos CCSP podem rodar automaticamente com --execute para esta rotina aprovada.','Sem Telegram direto.','Guardrail CCSP continua bloqueando status legado.']},args)
def add_auth(x):
 x.add_argument('--execute',action='store_true'); x.add_argument('--timeout',type=int,default=180); x.add_argument('--json',action='store_true')
def parser():
